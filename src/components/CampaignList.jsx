...
import { ArrowLeft, Home, PhoneIncoming, PhoneOutgoing, Phone, Clock, CheckCircle, TrendingUp, Plus, Pause, Play, Loader2, AlertCircle } from 'lucide-react'
...
const CampaignList = ({ type, campaigns: propCampaigns, onSelectCampaign, onBack, onHome, onCreateCampaign, onToggleCampaignStatus }) => {
  ...
  // Fetch campaigns when type changes
  useEffect(() => {
      ...
      try {
        ...
        if (result && result.campaigns) {
          ...
          const transformedCampaigns = result.campaigns.map(campaign => ({
            ...
            totalCalls: campaign.metrics?.totalCalls || 0,
            successfulCalls: campaign.metrics?.successfulCalls || 0,
            avgDuration: campaign.metrics?.avgDuration ? formatDuration(campaign.metrics.avgDuration) : '0:00',
            callLogs: [], // Will be populated if needed
